...
-- schumbar
-- Homework 2: RandomArt.hs
-- I worked with Arturo and got help from the MSI tutor, Lucas.
--}
...
data Expr
  ...
  | Op1     Expr Expr Expr      -- User made Operation number 1
  | Op2     Expr Expr           -- User made Operation number 2
  deriving (Show)

--------------------------------------------------------------------------------
-- | Some sample Expressions ---------------------------------------------------
--------------------------------------------------------------------------------
-- Define every single type of the expression
sampleExpr0 :: Expr
...
exprToString (Times e1 e2)        = exprToString e1 ++ "*" ++ exprToString e2 -- "(" ++ exprToString e1 ++ "*" ++ exprToString e2 ++ ")"
exprToString (Thresh e1 e2 e3 e4) = "(" ++ exprToString e1 ++ "<" ++ exprToString e2 ++ "?" ++ 
                                    exprToString e3 ++ ":" ++ exprToString e4 ++")"
exprToString (Op1 e1 e2 e3)       = "sin(pi*("++ exprToString e1 ++ "+" ++ exprToString e2 
                                              ++ "+" ++ exprToString e3 ++ "))"
exprToString (Op2 e1 e2)          = "cos(pi*("++ exprToString e1 ++ "+" ++ exprToString e2 ++ "))"
...
eval x y (Op1 e1 e2 e3)       = sin(pi*((eval x y e1) + (eval x y e2) + (eval x y e3)))
eval x y (Op2 e1 e2)          = cos(pi*((eval x y e1) + (eval x y e2)))
...
build d
  ...
  | (r == 6)  = Op1 (build(d-1)) (build(d-1)) (build(d-1))
  | (r == 7)  = Op2 (build(d-1)) (build(d-1))
